#!/usr/bin/env bash
# Train hybrid_coat_deep on fold0 with warmup+cosine — v3 fallback conservative run.
# Same as v2, but higher LR and lower stochastic depth.
...
DROP_PATH_RATE="${DROP_PATH_RATE:-0.05}"

SANITY="${SANITY:-0}"
RUN_DIR="${RUN_DIR:-coat_deep_v3_fold0}"
...
cd "${REPO_ROOT}"
echo "Training ${MODEL} on fold0 (coat_deep v3)"
